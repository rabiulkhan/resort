                                ...
                                style="border-radius:3px !important;margin-top:10px !important;" 
                                        class="btn btn-warning" 
                                            data-target="#demo-lg-modal" data-toggle="modal" 
                                                onClick="modal_form('food_package_add','1')">
                            <i class="entypo-user-add"></i> Create Food Package</button>
                        </div>
                    ...
                    <div class="panel-body">
                        <div class="table-responsive">
                            <table class="table table-hover table-striped table-vcenter">
								...
								<tbody>
									<?php foreach($food_package as $row)
										{ 
									?>
									<tr>
										<td><?php echo $this->crud_model->get_type_name_by_id('food_item', $row['food_item']); ?></td>
										<td><?php echo $row['quantity'];?></td>
										
										<td class="min-w-td"><?php echo $row['price'];?></td>
										
										</td>
										<td class="min-w-td text-center">
											<a class="btn btn-sm btn-trans btn-icon btn-hover-success fa fa-pencil add-tooltip" href="#" 
												data-original-title="Edit" data-target="#demo-lg-modal" data-toggle="modal" onClick="modal_form('food_package_edit',<?php echo $row['food_package_id']; ?>)">
											</a>
											<a class="btn btn-sm btn-trans btn-icon btn-hover-danger fa fa-times add-tooltip" onclick="return confirm('Are you sure?');"
												href="<?php echo base_url();?>index.php/admin/food_package/delete/<?php echo $row['food_package_id']; ?>" data-original-title="Delete">
											</a>
											<a class="btn btn-sm btn-trans btn-icon btn-hover-warning fa fa-lock add-tooltip"
												href="#" data-original-title="View" data-target="#demo-lg-modal" data-toggle="modal" 
                                                        onClick="modal_form('food_package_view',<?php echo $row['food_package_id']; ?>)" >
											</a>
										</td>
									</tr>
									<?php
