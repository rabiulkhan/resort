                                ...
                                style="border-radius:3px !important;margin-top:10px !important;" 
                                        class="btn btn-warning" 
                                            data-target="#demo-lg-modal" data-toggle="modal" 
                                                onClick="modal_form('food_item_add','1')">
                            <i class="entypo-user-add"></i> Create Food Item</button>
                        </div>
                    ...
                    <div class="panel-body">
                        <div class="table-responsive">
                            <table class="table table-hover table-striped table-vcenter">
								...
								<tbody>
									<?php foreach($food_item as $row)
										{ 
									?>
									<tr>
										<td>
                                            <img src="<?php echo base_url(); ?>upload/food_item_image/food_item_<?php echo $row['food_item_id']; ?>.jpg" class="img-sm img-circle" width="130" />
                                        </td>
                                        <td><?php echo $row['name']; ?></td>
                                        <td><?php echo $row['availability']; ?></td>
										<td><?php echo $this->crud_model->get_type_name_by_id('food_item_category', $row['food_item_category']); ?></td>
										
										</td>
										<td class="min-w-td text-center">
											<a class="btn btn-sm btn-trans btn-icon btn-hover-success fa fa-pencil add-tooltip" href="#" 
												data-original-title="Edit" data-target="#demo-lg-modal" data-toggle="modal" onClick="modal_form('food_item_edit',<?php echo $row['food_item_id']; ?>)">
											</a>
											<a class="btn btn-sm btn-trans btn-icon btn-hover-danger fa fa-times add-tooltip" onclick="return confirm('Are you sure?');"
												href="<?php echo base_url();?>index.php/admin/food_item/delete/<?php echo $row['food_item_id']; ?>" data-original-title="Delete">
											</a>
											<a class="btn btn-sm btn-trans btn-icon btn-hover-warning fa fa-lock add-tooltip"
												href="#" data-original-title="View" data-target="#demo-lg-modal" data-toggle="modal" 
                                                        onClick="modal_form('food_item_view',<?php echo $row['food_item_id']; ?>)" >
											</a>
										</td>
									</tr>
									<?php
